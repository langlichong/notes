 ...
 * TCP报文 = MBAP报文头（长度 7 字节） + PDU(protocol data unit, 功能码(1字节) 和 数据（长度不固定）)
...
public class ModbusTcpAndJ2mod {
     ...
     * Tx: 00 00 00 00 00 06 01 01 00 00 00 05  :  事务标识(2字节) 协议标识(2字节)  PDU数据长度(2字节) 单元标识（1字节） 功能码(1字节)  其实地址(2字节)  读取数量(00 005)
